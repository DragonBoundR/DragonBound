                        ...
                        self.gameserver.db
                          ...
                          .then(function (data) {
                            if (data.error_mysql || data.error_querry) {
                              ...
                              // NUEVO: Equipar automáticamente el avatar
                              const avatarType = item_data[2]; // 0=head, 1=body, 2=eyes, 3=flag, 4=background, 5=foreground
                              
                              // Actualizar el avatar equipado según el tipo
                              if (avatarType === 0) {
                                self.player.ahead = id;
                              } else if (avatarType === 1) {
                                self.player.abody = id;
                              } else if (avatarType === 2) {
                                self.player.aeyes = id;
                              } else if (avatarType === 3) {
                                self.player.aflag = id;
                              } else if (avatarType === 4) {
                                self.player.abackground = id;
                              } else if (avatarType === 5) {
                                self.player.aforeground = id;
                              }
                              
                              // Actualizar en la base de datos
                              const equipData = {
                                head: self.player.ahead,
                                body: self.player.abody,
                                eyes: self.player.aeyes,
                                flag: self.player.aflag,
                                background: self.player.abackground,
                                foreground: self.player.aforeground,
                              };
                              
                              self.gameserver.db.updateUserAvatarEquipById(equipData, self.player.reg_id)
                                .then(function() {
                                  // Actualizar atributos del avatar
                                  ...
                                  // Actualizar la lista de avatares
                                  self.gameserver.db
                                    .getPlayerAvatars(self)
                                    .then(function (avatarData) {
                                      if (!avatarData.error_mysql && !avatarData.error_querry) {
                                        var dat = self.gameserver.avatars.getAvatarDataList(
                                          avatarData.data_list
                                        );
                                        self.sendMessage(new Message.myAvatars(self, dat));
                                      }
                                    });
                                  
                                  // Si está en una sala, actualizar para todos
                                  if (self.room) {

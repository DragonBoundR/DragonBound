...
router.get("/bid", function (req, res) {
  if (
    typeof req.query.invite === "undefined" ||
    req.query.invite === "" ||
    req.query.invite === "0"
  ) {
    ...
    if (isNaN(req.query.invite)) {
      ...
      req.db.connection.getConnection().then((conn) => {
        ...
        conn
          .query("SELECT game_id, `rank`, IdAcc FROM users WHERE IdAcc = ?", [
            search,
          ])
          .then((rows) => {
            conn.release();
          ...
          .then((rows) => {
            ...
            if (rows[0].length > 0) {
              ...
              if (valid_enter === false) {
                ...
                Logger.log(
                  "Register Invite: '" + user + "' " + gender + " Exitoso."
                );
                req.db
                    ...
                    req.db
                              ...
                              .then(function (result2) {
                                ...
                                req.db
                                  ...
                                  .then(function (rows4) {
                                    ...
                                    conn
                                      .query(
                                        "SELECT Id, game_id, IdAcc FROM users WHERE IP = ?",
                                        [ip_result]
                                      )
                                      .then((rows2xD) => {
                                        conn.release();
                                        if (rows2xD[0].length === 1) {
                                          ...
                                          conn
                                            .query(
                                              "INSERT INTO guests SET from_id = ?, to_id = ?, check_ip = ?",
                                              [
                                                rowss[0].IdAcc,
                                                result[0].insertId,
                                                "green",
                                  ...
                                  .catch(function (err) {
                                    res.render("invitation", {
                                      ...
                                      rank: rowss[0].rank,
                                      error:
                                        '<div class="green buyroom">Error Servidor</div>',
                                    });
                                  });
                              })
                              .catch(function (err) {
                      ...
                      .catch(function (err) {
                        res.render("invitation", {
                          ...
                          rank: rowss[0].rank,
                          error:
                            '<div class="green buyroom">Pruebe con otro Usuario</div>',
                        });
                      });
                  })
                  .catch(function (err) {

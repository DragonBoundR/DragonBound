...
router.get("/u/:user_id/avatars", function (req, res) {
  ...
  req.db.connection.getConnection().then((conn) => {
    conn
      .query(
        "SELECT IdAcc, game_id, `rank`, gp, gold, cash, photo_url, bg_url, country FROM users WHERE IdAcc = ?",
        [parseInt(user_id)]
      )
      .then((rows1) => {
        conn.release();
        if (rows1[0].length > 0) {
          ...
          if (
            parseInt(req.session.account_id) === parseInt(rows[0].IdAcc) ||
            req.session.rank === 31
          ) {
            session_login = true;
          }
          ...
          if (rows[0].photo_url.length > 0) img = rows[0].photo_url;
          conn
            .query(
              "SELECT Id, UserId, aId, type, expire, is_cash, is_gift, gift_sent_by, amount, expire_time, date_ava_time, remove_ava FROM user_avatars WHERE UserId = ?",
              [parseInt(user_id)]
            )
            .then((rows2) => {
              conn.release();
              if (rows2[0].length > 0) {
                ...
                for (var i = 0; i < rows_x2.length; i++) {
                  ...
                  var type = "",
                    gender;
                  ava_date = ArrayToObject(ava_date, [
                    "id",
                    "type",
                    "gender",
                    "name",
                  ...
                  if (rows_x2[i].remove_ava === 0) texto_test += "<tr>";
                  else texto_test += '<tr class="Expired">';
                  var type =
                      ava_date.type == 0
                        ? "Head"
                        : ava_date.type == 1
                  ...
                  texto_test +=
                    "<td><nobr><script>TD(" +
                    Math.round(
                      new Date(parseInt(rows_x2[i].date_ava_time)).getTime() /
                        1000.0
                    ) +
                    ")</script></nobr></td>";
                  ...
                  if (rows_x2[i].is_cash === 1)
                    texto_test +=
                      '<td class="center"><div class="yes"></div></td>';
                  else texto_test += '<td class="center"></td>';
                  ...
                  if (rows_x2[i].remove_ava === 0)
                    ...
                    texto_test +=
                      '<td class="center"><div class="yes"></div></td>';
                  if (rows_x2[i].remove_ava === 0) texto_test += "<td></td>";
                  else
                    texto_test +=
                      '<td><form method="POST"><input type="hidden" name="csrfmiddlewaretoken" value="BKSBKASBDKABlansdkabs7dBAKBASBDbakbdJBAKSBDAJBSjabksdb"><input type="hidden" name="undelete_id" value="' +
                      rows_x2[i].Id +
                      '"><button>UnDelete</button></form></td>';
                  texto_test += "</tr>";
                }
...
router.post("/u/:user_id/avatars", function (req, res) {
  ...
  req.db.connection.getConnection().then((conn) => {
    conn
      .query("UPDATE user_avatars SET remove_ava = 0 WHERE Id = ?", [
        undelete_id,
      ])
      .then((rows1) => {
        conn.release();

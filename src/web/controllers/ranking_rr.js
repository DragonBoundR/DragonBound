...
router.get("/rr", function (req, res) {
  ...
  if (req.query.r == 8) {
    req.db.connection.getConnection().then((conn) => {
      ...
      conn
        .query(
          "SELECT Id, Name, members, `rank`, img, fondo, about FROM guild WHERE rank !=26 AND rank !=27 AND rank !=31 ORDER BY members DESC limit " +
            number_link +
            ", 30"
        )
        .then((rows) => {
    ...
    req.db.connection.getConnection().then((conn) => {
      ...
      conn
        .query(
          "SELECT u.game_id, u.gp, u.rank, g.Name FROM users u LEFT JOIN guild_member m ON m.UserId = u.IdAcc LEFT JOIN guild g ON g.Id = m.Id WHERE u.rank !=26 AND u.rank !=27 AND u.rank !=31 ORDER BY u.gp DESC limit " +
            number_link +
            ", 30"
        )
        .then((rows) => {
            ...
            conn
              ...
              .then((rows_2) => {
                ...
                if (res00.next_reset_rankings <= Date.now()) {
                  ...
                  conn
                    .query(
                      "SELECT u.game_id, u.gp, u.rank, u.IdAcc, g.Name FROM users u LEFT JOIN guild_member m ON m.UserId = u.IdAcc LEFT JOIN guild g ON g.Id = m.Id ORDER BY u.gp DESC limit 0, 1"
                    )
                    .then((rows_ranking1) => {
                      conn.release();
                      ...
                      conn
                        ...
                        .then((rows_ranking1_1) => {
                          ...
                          conn
                            .query(
                              "SELECT u.game_id, u.gp, u.rank, u.IdAcc, g.Name FROM users u LEFT JOIN guild_member m ON m.UserId = u.IdAcc LEFT JOIN guild g ON g.Id = m.Id ORDER BY u.gp DESC limit 1, 4"
                            )
                            .then((rows_ranking2) => {
                              conn.release();
                              ...
                              for (var xm_rows2 in ranking_rows2) {
                                conn
                                  ...
                                  .then((rows_ranking2_1) => {
                                    ...
                                    conn
                                      .query(
                                        "SELECT u.game_id, u.gp, u.rank, u.IdAcc, g.Name FROM users u LEFT JOIN guild_member m ON m.UserId = u.IdAcc LEFT JOIN guild g ON g.Id = m.Id ORDER BY u.gp DESC limit 5, 16"
                                      )
                                      .then((rows_ranking3) => {
                                        conn.release();
    ...
    req.db.connection.getConnection().then((conn) => {
      ...
      conn
        .query(
          "SELECT u.game_id, u.rank, u.prixw, g.Name FROM users u LEFT JOIN guild_member m ON m.UserId = u.IdAcc LEFT JOIN guild g ON g.Id = m.Id WHERE u.prixw != 0 AND u.rank !=26 AND u.rank !=27 AND u.rank !=31 ORDER BY u.prixw DESC limit " +
            number_link +
            ", 30"
        )
        .then((rows) => {
    ...
    req.db.connection.getConnection().then((conn) => {
      ...
      conn
        .query(
          "SELECT Id, Name, points, rank FROM guild WHERE points != 0 AND rank !=26 AND rank !=27 AND rank !=31 ORDER BY points DESC LIMIT " +
            number_link +
            ", 30"
        )
        .then((rows) => {

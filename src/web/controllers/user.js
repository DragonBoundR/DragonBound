        ...
        try {
            req.db.getUserByGameId(tmusr)
                .then(function(resb) {
				    ...
				    var player_cash = Commatize(rows[0].cash);
				    var player_gold = Commatize(rows[0].gold);
				    ...
				    var id_login = req.session.account_id;
				    if (req.session.account_id) {
					    login = true;
					    loguser = req.session.game_id;
					    if(guild_user_id === id_login)
						    var admin = true;
				    }
                    if (req.session.account_id) {
                        login = true;
                    ...
                    req.db.connection.getConnection().then(conn => {
                        ...
                        conn.query('SELECT Id, UserId, aId, type, expire, is_cash, is_gift, amount, expire_time FROM user_avatars WHERE UserId = ?', [guild_user_id])
                            .then(rows3 => { //rows3[0].length
								...
								conn.query('SELECT * FROM games WHERE user_id = ? ORDER BY freg ASC', [rows[0].IdAcc]).then(rows6 => {
                                conn.release();
                                var datos = [];
                                for(var i=0;i<rows6[0].length;i++){

                                   datos.push(rows6[0][i]);
                                }
                                //console.log(datos,rows6);
                                var actualh = Math.floor(Date.now()/3600000);
                                var inicioh = actualh-336;//336;
                                var grafico = [],grafico_index = {};
                                for(var i=inicioh;i<=actualh;i++){
                                    grafico_index[i] = grafico.length;
                                    grafico.push([i,0,0,0]);
                                }
                                for(var i=0;i<datos.length;i++){
                                    var dato = datos[i];
                                    var graf = grafico[grafico_index[Math.floor(dato.freg/3600000)]];
                                    if(graf){
                                        graf[1] += parseInt(dato.gp);
                                        graf[dato.win==1 ? 2 : 3]++;
                                    }
                                }
...
texto_test += html;
                                                        ...
                                                        res.render('info', {
                                                            ...
                                                            guild: guild_name,
                                                            pos_state: true,
															gp_graph:JSON.stringify(grafico),
                                                            adminPage: rows[0].rank < 25,

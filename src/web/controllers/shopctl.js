  ...
  if (Buffer.byteLength(guild_name, "utf8") < 30) {
    try {
      req.db.connection.getConnection().then((conn) => {
        conn
          .query(
            "SELECT game_id, `rank`, cash, IdAcc FROM users WHERE IdAcc = ?",
            [user_id]
          )
          .then((rows0) => {
            conn.release();
        ...
        conn
          .query(
            "SELECT u.game_id, u.IdAcc, gm.Job FROM users u INNER JOIN guild_member gm ON u.IdAcc = gm.UserId INNER JOIN guild g ON gm.Id = g.Id WHERE u.IdAcc = ? AND g.Name = ?",
            [user_id, tmusr]
          )
          .then((rows0_1) => {
            conn.release();
        ...
        conn
          .query("SELECT Id, Name, img, fondo FROM guild WHERE Name = ?", [
            tmusr,
          ])
          .then((rows) => {
            conn.release();
            if (rows[0].length > 0) {
              ...
              conn
                .query(
                  "SELECT g.Id, g.Name, g.img, g.fondo, gc.user_id, gc.time_coin, gc.date_coin, gc.coin_img, u.game_id FROM guild g INNER JOIN guild_coins gc ON g.Id = gc.guild_id INNER JOIN users u ON gc.user_id = u.IdAcc WHERE g.Name = ?",
                  [rowsxd[0].Name]
                )
                .then((rows1) => {
                  conn.release();
                  if (rows1[0].length > 0) {
                    ...
                    for (ix2 = 0; ix2 < rows_x1.length; ix2++) {
                      purchase_history +=
                        '<tr><td><nobr><a href="/user/' +
                        rows_x1[ix2].game_id +
                        '">' +
                        rows_x1[ix2].game_id +
                        '</a></nobr></td><td class="nowarp"><span class="tokens">1x</span> ' +
                        getDateTime(rows_x1[ix2].time_coin) +
                        ".</td></tr>";
                    }
                    for (ix3 = 0; ix3 < rows_x1.length; ix3++) {
                      user_feature +=
                        ...
                        rows_x1[ix3].game_id +
                        "</a></nobr></td><td>" +
                        rows_x1[ix3].date_coin +
                        '</td><td class="tokens center">1</td><td class="nowarp">' +
                        getDateTime(rows_x1[ix3].time_coin) +
                        ".</td></tr>";
                    }
                    if (guild_leader === 1) leader_status = true;
                    res.render("shop", {
                      guild: rowsxd[0].Name,
                      recargar: "",
                      img: rowsxd[0].img,
                    ...
                    res.render("shop", {
                      ...
                      fondo: rowsxd[0].fondo,
                      hisoty_purchase:
                        "<tr><td>Nobody bought coins yet, be the first one</td></tr>",
                      feature_user:
                        "<tr><td></td><td>No purchases yet, ask guild leader to buy something</td></tr>",
                      login: login,
                      login_id: loguser,
                      guild_coins: 0,
            ...
            if (rows[0].length > 0) {
              ...
              if (Buffer.byteLength(guild_name, "utf8") < 30) {
                try {
                  conn
                    .query(
                      "SELECT Id, Name, img, fondo FROM guild WHERE Name = ?",
                      [tmusr]
                    )
                    .then((rows2) => {
                      conn.release();
                      if (rows2[0].length > 0) {
                        ...
                        if (guild_action === "BuyTokens") {
                          ...
                          if (
                            ...
                            error_primario =
                              "Not enough Cash, you have only " +
                              Commatize(rows_x1[0].cash) +
                              " Cash. buy some more and try again";
                            cont = false;
                          } else if (rows_x1[0].rank < 17) {
                            error_primario =
                              "Sorry, you need to be rank red wand or above to buy guild coins.";
                            cont = false;
                          } else {
                          }
                          ...
                          if (cont) {
                            conn
                              .query(
                                "UPDATE users SET cash = cash - ? WHERE IdAcc = ?",
                                [10000, user_id]
                              )
                              .then((rows_x3) => {
                                conn.release();
                                if (rows_x3.length > 0) {
                                  ...
                                  Logger.info(
                                    "The user " +
                                      ...
                                      " is discounted " +
                                      10000 +
                                      " thousand of cash for the purchase of guild coins."
                                  );

                                  conn
                                    .query(
                                      "INSERT into guild_coins SET guild_id = ?, user_id = ?, time_coin = ?, date_coin = ?",
                                      [
                                        rows_x2[0].Id,
                                        user_id,
                                        time_coin,
                                    ...
                                    .then((rows_4) => {
                                      ...
                                      if (rows_4.length > 0) {
                                        ...
                                        Logger.info(
                                          "The " +
                                            rows_x2[0].Name +
                                            " guild successfully won a new currency."
                                        );
                            ...
                            .then((rows_x3) => {
                              ...
                              if (rows_x3[0].length > 0) {
                                ...
                                conn
                                  ...
                                  .then((rows_x4) => {
                                    ...
                                    if (rows_x4[0].length > 0) {
                                      ...
                                      conn
                                        .query(
                                          "SELECT id, guild_id, user_id, time_coin, date_coin, coin_img FROM guild_coins WHERE guild_id = ? ORDER BY rand() LIMIT ?",
                                          [rows_x2[0].Id, 1]
                                        )
                                        .then((rows2) => {
                                          conn.release();
                                          if (rows2[0].length > 0) {
                                            ...
                                            Logger.info(
                                              "ID Mostrado para eliminar coins es: " +
                                                guild2_id
                                            );

                                            conn
                                              .query(
                                                "SELECT gm.Id, gm.UserId, gm.Job, u.game_id FROM guild_member gm INNER JOIN users u ON gm.UserId = u.IdAcc WHERE gm.Id = ? AND u.game_id = ?",
                                                [rows_x2[0].Id, guild_info]
                                              )
                                              .then((rows3) => {
                                                conn.release();
                                                if (rows3[0].length > 0) {
                                                  ...
                                                  conn
                                                    .query(
                                                      "DELETE FROM guild_coins WHERE id = ?",
                                                      [guild2_id]
                                                    )
                                                    .then((rows4) => {
                                                      conn.release();
                                                      if (
                                                        rows4[0].affectedRows >
                                                          0 ||
                                                        rows4[0].changedRows > 0
                                                      ) {
                                                        Logger.info(
                                                          "Guild Coin Eliminado"
                                                        );
                                                        var Job = 2;
                                                        var userid =
                                                          res0000[0].UserId;
                                                        ...
                                                        conn
                                                          .query(
                                                            "UPDATE guild_member SET Job = ? WHERE UserId = ?",
                                                            [Job, userid]
                                                          )
                                                          .then((rows4) => {
                                                            conn.release();
                                                            if (
                                                              ...
                                                              Logger.info(
                                                                "El user: " +
                                                                  res0000[0]
                                                                    .game_id +
                                                                  " Ahora es semilider en el guild: " +
                                                                  rows_x2[0]
                                                                    .Name
                                                              );
                                                              res.redirect(
                                                              ...
                                                              Logger.info(
                                                                "Error al poner como semilider en el guild: " +
                                                                  res0[0]
                                                                    .guild_id
                                                              );
                                                              res.redirect(
                                                        ...
                                                        Logger.info(
                                                          "Error al eliminar un coin en el guild: " +
                                                            res0[0].guild_id
                                                        );
                                                        res.redirect(
                                                          req.header("Referer")
                                      ...
                                      res.render("shop", {
                                        ...
                                        error_x2: "",
                                        error_x3:
                                          "You are not the leader of this guild to be able to place this order",
                                        leader_date: false,
                                      });
                                      /* |<*-================-*>| */
                                    }
                                ...
                                res.render("shop", {
                                  ...
                                  error_x2: "",
                                  error_x3:
                                    "Not enough Guild Coins, buy some more and try again",
                                  leader_date: false,
                                });
                                /* |*=====---------------------=====*| */
                              }
                        ...
                        res.send(
                          JSON.stringify("Oops An undefined error occurred")
                        );
                      }
                    });
                } catch (e) {
